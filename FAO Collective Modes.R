...
#--
u <- ExportData$Value
ExportData$Value[which(is.na(u) == T | is.nan(u) == T)] <- 0
...
foodbal_items_keep <- c("Grand Total",
                        "Animal Products",
...
FoodBal <- subset(FoodBal, Item %in% foodbal_items_keep)
u <- FoodBal$Item
...
unique(FoodBal$Element)
df <- subset(FoodBal, Element %in% c("Stock Variation", "Production", "Import Quantity", "Export Quantity", "Use"))
